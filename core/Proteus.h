...
enum vals {toGiven=0, toWorldCtxt=0x0100, toHomePos=0x0200, fromHere=0x0300, asFunc=0x0400, intersect=0x0500, asTag=0x0600, asNone=0x0700, // TODO B4: remove these
    ...
    tType=3, tUnknown=0, tUInt=1, tString=2, tList=3,   notLast=(0x04<<goSize), tReal=0x04,
...
inline fix16_t getReal(infon* inf) {/*normalize(inf);  */           \
  if((inf->pFlag&tType)==tUInt && !(inf->pFlag&fUnknown)) return (fix16_t)inf->value; else return 0;}
